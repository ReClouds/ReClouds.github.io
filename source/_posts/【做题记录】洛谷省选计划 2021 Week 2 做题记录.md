...
[代码实现](https://www.luogu.com.cn/record/102435013)
...
[代码实现](https://www.luogu.com.cn/record/102442895)
...
- [CF603E Pastoral Oddities](https://codeforces.com/problemset/problem/603/E) {% label AC green %}
...
- [CF797E Array Queries](https://codeforces.com/problemset/problem/797/E) {% label AC green %}

题解：

{% note success flat %}

根号分治，对于 $k > \sqrt{n}$ 的部分暴力做，对于 $k \le \sqrt{n}$ 的部分预处理即可。

[代码实现](https://codeforces.com/contest/797/submission/193830742)

{% endnote %}

------

- [CF1039D You Are Given a Tree](https://codeforces.com/problemset/problem/1039/D) {% label AC green %}

题解：

{% note success flat %}

首先，对于固定的 $k$，存在 $\mathcal{O}(n)$ 的贪心做法。

考虑数据分治，设定阈值 $S$，对于 $k \le S$ 的部分暴力计算，对于 $k >S$ 的部分由于答案不增且不会超过 $\displaystyle \frac{n}{S}$，因此我们二分求出每个答案对应的 $k$ 的区间即可。

平衡后的时间复杂度为 $\mathcal{O}(n\sqrt{n\log_2n})$。

注意这题卡常，可以考虑确定根之后将边数砍掉一半，以及记录 $dfn$ 序来避免 $\mathcal{DFS}$ 的递归常数。

[代码实现](https://codeforces.com/contest/1039/submission/193944823)
