...
- [P7136 [THUPC2021 初赛] 方格游戏](https://www.luogu.com.cn/problem/P7136) {% label AC green %}
...
对于一个上下长为 $len$ 的障碍，它的贡献为 $\displaystyle 2 \cdot \sum_{i = 1}^{len}\sum_{j = 1}^{len} \min (i, len - i +1, j, len - j +1)$。

考虑偶数的情况，设 $len' = \dfrac{len}{2}$，那么原式可以化为 $\displaystyle 8 \cdot \sum_{i = 1}^{len} \sum_{j = 1}^{len} \min (i, j)$，这个对每个数讨论其作为 $\min(i, j)$ 的次数即可。奇数的情况仅需多讨论一种很小的情况即可。

[代码实现](https://www.luogu.com.cn/record/104556034)

{% endnote %}

------

- [CF662C Binary Table](https://codeforces.com/problemset/problem/662/C) {% label AC green %}
...
对于一列的初始状态 $T$，它的贡献为 $0$ 和 $1$ 的出现次数的较小值，记其为 $f_T$。注意到一列的初始状态 $T$ 在行的翻转 $S$ 过后得到的状态为 $T~\texttt{xor}~S$，不妨设初始状态 $T$ 的出现次数为 $g_T$，设翻转状态 $S$ 的答案为 $ans_S$，那么有 $\displaystyle ans_S = \sum_{T~\texttt{xor}~U = S}f_T \cdot g_U$，这其实就是一个异或卷积，用 $\mathcal{FWT}$ 求解即可。

[代码实现](https://codeforces.com/contest/662/submission/189693226)

{% endnote %}

------

- [P4491 [HAOI2018] 染色](https://www.luogu.com.cn/problem/P4491) {% label AC green %}

题解：

{% note success flat %}

设 $f_i$ 表示至少有 $i$ 种颜色满足要求的方案数，$g_i$ 表示恰好有 $i$ 种颜色满足要求的方案数。

$f_i$ 可以简单计算，列出二者的关系式也不难，二项式反演后用 $\mathcal{NTT}$ 加速计算即可。

[代码实现](https://www.luogu.com.cn/record/103361674)

{% endnote %}

------

- [P7736 [NOI2021] 路径交点](https://www.luogu.com.cn/problem/P7736) {% label AC green %}

题解：

{% note success flat %}

设 $\operatorname{sign}(x)$ 表示 $(-1)^x$。

可以发现对于任意两条路径 $\text{P},\text{Q}$，所有 $i \in (1, k)$ 对应的 $\operatorname{sign}(\text{P}_i - \text{Q}_i)$ 都会贡献两遍，也就是说，答案仅和起点和终点有关。

套上 $\mathcal{LGV}$ 引理的板子即可。

[代码实现](https://www.luogu.com.cn/record/103542625)

{% endnote %}

------

- [P5516 [MtOI2019] 小铃的烦恼](https://www.luogu.com.cn/problem/P5516) {% label AC green %}
...
先考虑如何求概率。设 $p_i$ 表示当前整个字符串包含 $i$ 个字符 $c$，最终能够到达目标状态的概率。转移显然有 $\displaystyle p_i = \frac{i \cdot (n - i)}{n \cdot (n - 1)}\cdot (p_{i - 1} +p_{i +1}) +(1 - \frac{2 \cdot i \cdot (n - i)}{n \cdot (n - 1)})\cdot p_i$，整理可得 $p_i = \dfrac{p_{i - 1}+p_{i +1}}{2}$，又根据 $p_0 = 0$ 和 $p_n = 1$ 可得 $p_i = \dfrac{i}{n}$。

再考虑如何求期望。设 $f_i$ 表示在能够到达目标状态的前提下，当前整个字符串包含 $i$ 个字符 $c$，最终到达目标状态的期望步数。注意由于 $f_i$ 是条件期望，因此转移时还需乘上相应的概率，即 $\displaystyle f_i = \frac{i - 1}{2 \cdot i} \cdot f_{i - 1} +\frac{i+ 1}{2\cdot i}\cdot f_{i+ 1} + \frac{n\cdot (n - 1)}{2\cdot i \cdot (n - i)}$（注意要特判 $f_1$ 的转移）。求解方程组可以用高斯消元做到 $\mathcal{O}(n^3)$，但由于转移实际上是链状的，并且 $f_n=0$，因此可以用待定系数法做到 $\mathcal{O}(n)$。

[代码实现](https://www.luogu.com.cn/record/103848670)

{% endnote %}

------

- [P6630 [ZJOI2020] 传统艺能](https://www.luogu.com.cn/problem/P6630) {% label AC green %}
...
每个时刻，每个节点存在三种状态：自己存在标记、自己不存在标记但自己的祖先存在标记以及自己和自己的祖先都不存在标记。我们可以很容易的求出它们之间的转移系数，这样我们就得到了一个 $\mathcal{O}(nk)$ 的做法。

状态只有三种，转移式是线性的，转移系数是固定的。综合这三点，我们用矩阵快速幂加速转移即可。

[代码实现](https://www.luogu.com.cn/record/104439445)

{% endnote %}

------

- [P6624 [省选联考 2020 A 卷] 作业题](https://www.luogu.com.cn/problem/P6624) {% label AC green %}
...
对于一条边，设其原本的边权为 $w$，我们不妨将其表示为二项式 $w\cdot x +1$，然后根据矩阵树定理对基尔霍夫矩阵求行列式，那么答案即为行列式的 $x$ 项系数。

[代码实现](https://www.luogu.com.cn/record/104449012)

{% endnote %}

------

- [P4707 重返现世](https://www.luogu.com.cn/problem/P4707) {% label AC green %}

题解：

{% note success flat %}

考虑把求第 $k$ 小转化为求第 $n - k + 1$ 大，根据扩展 $\min - \max$ 容斥，问题转化为求 $\displaystyle \sum_{T \ne \emptyset} (-1)^{|T| + k} \cdot \binom{|T| - 1}{k - 1}\cdot \text{E}(\min(T))$。

这个问题直接做必然是不行的，不过我们可以注意到 $m$ 很小，并且有 $\text{E}(\min(T)) = \dfrac{m}{\displaystyle \sum_{i \in T} p_i}$，那么我们可以考虑把 $\displaystyle \sum_{i \in T} p_i$ 记入状态中，跑一个背包 $\mathcal{DP}$。

然而直接做还需要记录 $|T|$ 这一维，不过由于 $\displaystyle \binom{|T| - 1}{k - 1} = \binom{|T| - 2}{k - 1} +\binom{|T| - 2}{k - 2}$，那么我们多记录 $k$ 这一维即可省掉 $|T|$ 这一维，由于 $k$ 很小，因此时间复杂度是可接受的。

[代码实现](https://www.luogu.com.cn/record/104464458)

{% endnote %}

------

- [P4091 [HEOI2016/TJOI2016] 求和](https://www.luogu.com.cn/problem/P4091) {% label AC green %}
...
$$
\begin{aligned}
&\sum_{i = 0}^n \sum_{j = 0}^i 2^j \cdot j! \cdot \begin{Bmatrix}i \\ j\end{Bmatrix}
\\
= &\sum_{j = 0}^n 2^j \cdot j! \sum_{i = 0}^n\begin{Bmatrix}i \\ j\end{Bmatrix}
\\
= &\sum_{j = 0}^n 2^j \cdot j! \sum_{i = 0}^n \sum_{k = 0}^j \frac{k^i}{k!} \cdot \frac{(-1)^{j - k}}{(j - k)!}
\\
= &\sum_{j = 0}^n 2^j \cdot j! \sum_{k = 0}^j \frac{(-1)^{j - k}}{k! \cdot (j - k)!} \sum_{i = 0}^n k^i
\\
= &\sum_{j = 0}^n 2^j \cdot j! \sum_{k = 0}^j \frac{(-1)^{j - k}}{(j - k)!} \cdot \frac{k^{n + 1} - 1}{k!\cdot(k - 1)}
\end{aligned}
$$
可以发现后面的部分呈卷积形式，用 $\mathcal{NTT}$ 优化即可。

[代码实现](https://www.luogu.com.cn/record/78608821)
