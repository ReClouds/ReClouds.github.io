...
\\
&= \sum_{i = 1} ^n \sum_{d | i} f(d) \cdot g(\frac{i}{d})
\\
...
$$
g(1)\cdot S(n) = \sum_{d = 1}^ng(d)\cdot S(\lfloor\frac{n}{d}\rfloor) - \sum_{d = 2}^ng(d)\cdot S(\lfloor\frac{n}{d}\rfloor)
$$

即：

$$
S(n) = \frac{\displaystyle S'(n)- \sum_{d = 2}^ng(d)\cdot S(\lfloor\frac{n}{d}\rfloor)}{g(1)}
$$
...
对复杂度及其证明感兴趣的读者可以参考国家队爷朱震霆于 $2018$ 年的国家集训队论文。
