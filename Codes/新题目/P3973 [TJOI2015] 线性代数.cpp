...
namespace Program
{
    ...
    i64 m, C[MAXN], B[MAXN][MAXN], w[MAXM], sum;
    bool p[MAXM];
    vector<int> vec[MAXM];
    ...
    {
        int S, T, tot = 1, hd[MAXM], nw[MAXM], nxt[MAXM << 1], to[MAXM << 1], dep[MAXM];
        i64 cap[MAXM << 1], mxf;
    ...
    inline int Run()
    {
        ...
        for(register int i = 1; i <= n; i++)
        {
            ...
            {
                B[i][j] = Read32(), id[i][j] = ++cnt;
                if(i != j) w[id[i][j]] = B[i][j];
            }
        }
        for(register int i = 1; i <= n; i++) C[i] = Read32(), w[mp[i] = ++cnt] = B[i][i] - C[i];
        for(register int i = 1; i <= n; i++)
            for(register int j = 1; j <= n; j++) 
                if(i != j) Link(id[i][j], mp[i]), Link(id[i][j], mp[j]);
