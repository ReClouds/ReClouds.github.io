...
namespace Program
{
    ...
    int n, cnt, a[MAXN], id[MAXN][MAXN], mp[MAXN];
    i64 m, d[MAXN][MAXN], w[MAXM], sum;
    bool p[MAXM];
    vector<int> vec[MAXM];

    inline void Link(int u, int v) { return vec[u].push_back(v), void(); }
    ...
    namespace Network_Flow
    {
        ...
        inline void Init()
        {
            ...
            {
                if(w[i] > 0) Link(S, i, w[i]), sum += w[i];
                else if(w[i] < 0) Link(i, T, -w[i]);
            }
            for(register int i = 1; i <= cnt; i++)
                for(register vector<int> :: iterator v = vec[i].begin(); v != vec[i].end(); v++) Link(i, *v, INF);
            return;
        }
    ...
    {
        n = Read32(), m = Read64();
        for(register int i = 1; i <= n; i++) a[i] = Read32(), p[a[i]] = true;
        for(register int i = 1; i <= n; i++) 
        {
            for(register int j = i; j <= n; j++)
            {
                d[i][j] = Read64(), id[i][j] = ++cnt, w[id[i][j]] = d[i][j];
                if(i == j) w[id[i][j]] -= a[i];
            }
        }
        for(register int i = 1; i < MAXM; i++) if(p[i]) w[++cnt] = -m * i * i, mp[i] = cnt;
        for(register int i = 1; i <= n; i++) Link(id[i][i], mp[a[i]]);
        for(register int i = 1; i < n; i++)
            for(register int j = i + 1; j <= n; j++)
                Link(id[i][j], id[i + 1][j]), Link(id[i][j], id[i][j - 1]);
        nf :: Init(), nf :: Dinic(), sum -= nf :: mxf;
        Write64(sum);
        return 0;
    }
}
