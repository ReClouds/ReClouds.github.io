...
namespace Program
{
    ...
    inline int Run()
    {
        n = Read32(), m = Read32(), g = Read64();
        for(register int i = 1; i <= n; i++) a[i] = Read32();
        for(register int i = 1; i <= n; i++) v[i] = Read64();
        nf :: S = n + m + 1, nf :: T = n + m + 2;
        for(register int i = 1; i <= n; i++)
        {
            if(!a[i]) nf :: Link(nf :: S, i, v[i]);
            else nf :: Link(i, nf :: T, v[i]);
        }
        i64 ans = 0;
        for(register int i = 1; i <= m; i++)
        {
            int b = Read32(); i64 w = Read64(); int k = Read32();
            ans += w;
            for(register int j = 1; j <= k; j++)
            {
                int t = Read32();
                if(!b) nf :: Link(n + i, t, INF);
                else nf :: Link(t, n + i, INF);
            }
            int p = Read32();
            w += g * p;
            if(!b) nf :: Link(nf :: S, n + i, w);
            else nf :: Link(n + i, nf :: T, w);
        }
        nf :: Dinic();
        Write64(ans - nf :: mxf);
        return 0;
